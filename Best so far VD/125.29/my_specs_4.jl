...
student_names=["Arjun" "Thomas"]
student_numbers=["000000001" "000000002"]
group_name=["Dennit Racing"]

engine_type = 1 # Enter your engine choice here
# 1 means engine 1 (higher redline)
# 2 means engine 2 (more torque)

e = [3.27,2.46,1.94,1.74,1.64] # Enter your 5 or 6 gear ratios here, starting from _first_ gear, i.e., in the default values, 1st gear is 5.0:1, note that the defaults are a poor choice
ex = 4 # Enter your axle final drive ratio here
...
front_uaapr = [-0.15,-0.4,0.45] # lower a-arm pivot, rear
front_laapf = [0.15,-0.5,0.15] # upper a-arm pivot, front
...
rear_ubj= [0,-0.15,0.40] # upper ball joint
rear_lbj = [0,-0.1,0.10] # lower ball joint
rear_uaapf = [0.15,-0.5,0.40] # upper a-arm pivot, front
rear_uaapr = [-0.15,-0.5,0.40] # lower a-arm pivot, rear
rear_laapf = [0.15,-0.6,0.10] # upper a-arm pivot, front
rear_laapr = [-0.15,-0.6,0.10] # lower a-arm pivot, rear
rear_itre = [-0.1,-0.45,0.25] # inner tie rod end
rear_otre = [-0.1,-0.12,0.25] # outer tie rod end

rear_sle = [0,-0.2,0.10] # spring lower end
rear_sue = [0,-0.3,0.25]  # spring upper end


# braking system parameters
fbf = 0.45 # front brake fraction, from 0 to 1
# 0 means rear brakes only
# 1 means front brakes only
# 0.5 equal brake torques on both axles

# driver model
acc_max = 0.599 # [g's] maximum acceleration, used to compute target speeds based on track curvature