...
student_names=["Arjun Posarajah" "Thomas Recchia"]
student_numbers=["104980541" "104969537"]
group_name=["Dennit Racing"]

engine_type = 2 # Enter your engine choice here
# 1 means engine 1 (higher redline)
# 2 means engine 2 (more torque)

e = [3.92,2.33,1.44,1,0.85] # Enter your 5 or 6 gear ratios here, starting from _first_ gear, i.e., in the default values, 1st gear is 5.0:1, note that the defaults are a poor choice
ex = 3.955 # Enter your axle final drive ratio here
...
rear_uaapr = [-0.15,-0.5,0.45] # lower a-arm pivot, rear
rear_laapf = [0.15,-0.6,0.15] # upper a-arm pivot, front
...
fbf = 0.6375 # front brake fraction, from 0 to 1
# 0 means rear brakes only
# 1 means front brakes only
# 0.5 equal brake torques on both axles

# driver model
acc_max = 0.6767# [g's] maximum acceleration, used to compute target speeds based on track curvature